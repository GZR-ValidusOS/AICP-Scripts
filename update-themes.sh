...
function copy_all () {
  ...
  do
    cd $WORKING_DIR/packages/apps
    cp -r ${array[i]} $WORKING_DIR/GZR-Scripts/themes-resources/packages/apps/
  done
}

declare -a root=('GZR-Scripts' 'abi' 'bionic' 'art' 'bootable' 'build' 'dalvik' 'development' 'device' 'external' 'frameworks' 'hardware' 'kernel' 'libcore' 'libnativehelper' 'manifest' 'ndk' 'out' 'packages' 'pdk' 'prebuilts'
...
declare -a values=('all_search_engines.xml' 'appmsg_colors.xml' 'arrays.xml' 'attrs_manifest.xml' 'bookmarks_icons.xml' 'bools.xml' 'config.xml' 'defaults.xml' 'donottranslate.xml' 'donottranslate_config.xml'
                   ...
                   'donottranslate-search_engines.xml' 'validus_arrays.xml' 'validus_strings.xml' 'validus_symbols.xml' 'fractions.xml' 'ids.xml' 'integers.xml' 'internal.xml' 'keys.xml' 'lland_config.xml' 'lland_strings.xml' 'plurals.xml'
...
cd values
delete_useless ${values[@]}
rm -rf  $WORKING_DIR/GZR-Scripts/themes-resources/frameworks
cd $WORKING_DIR
...
rm -rf GZR-Scripts/themes-resources/packages
mkdir GZR-Scripts/themes-resources/packages
mkdir GZR-Scripts/themes-resources/packages/apps
mkdir GZR-Scripts/themes-resources/frameworks
echo "Copying all files to $WORKING_DIR/GZR-Scripts/themes-resources"
#cp -r $WORKING_DIR/packages/apps/DU-Updater $WORKING_DIR/GZR-Scripts/themes-resources/packages/apps/
copy_all ${theme_packages[@]}
cp -r $WORKING_DIR/frameworks/base/core $WORKING_DIR/frameworks/base/packages $WORKING_DIR/GZR-Scripts/themes-resources/frameworks
cd /home/theringmaster/Projects/Validus/Build/GZR-Scripts/themes-resources
cd $WORKING_DIR/GZR-Scripts/themes-resources/packages/apps/Dialer/InCallUI
delete_useless ${res[@]}
cd $WORKING_DIR/GZR-Scripts/themes-resources
